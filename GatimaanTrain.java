...
public class GatimaanTrain {
    static ArrayList<GatimaanTrain> gatimaanTrains= new ArrayList<>();
    ...
    static void addTrain(String[] s) {
        GatimaanTrain tmp = new GatimaanTrain();
        tmp.TrainNo=s[1];
        tmp.LineNo=s[2];
        for (GatimaanTrain i : gatimaanTrains)
            ...
            if (i.TrainNo.equals(s)) {
                for (Line j : Line.lines) {
                    if(j.lineNo.equals(i.LineNo)){
                        for (GatimaanTrain k: j.GTrainsOnline) {
                            if(i.TrainNo.equals(k.TrainNo)){
                                j.numOfTrain--;
                                j.GTrainsOnline.remove(k);
                                gatimaanTrains.remove(i);
